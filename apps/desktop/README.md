...
The shell displays its embedded startup page immediately, accepts only the sidecar's exact loopback origin, denies popups and unexpected top-level navigation, and exposes no Tauri shell, filesystem, or general invoke API to the Web UI. A second launch focuses the existing window. On Windows, the window close button saves its state and hides it to the system tray; click the tray icon or choose **Show DeepSeek Harness** to restore it, and choose **Exit** from the tray menu to stop the application. Exit sends a protocol-level shutdown request and waits for Harness disposal; the Windows Job Object and macOS process group are timeout fallbacks. macOS keeps its normal window-close behavior.
