...
The sidecar embeds the Web backend, built-in plugins, and Web assets in its VFS. Target-specific `node-pty`, ripgrep, and process helpers remain ordinary sidecar files. Office document conversion stages the `@deepseek-ai/libreoffice-kit` API and its platform engine as ordinary resources under `resources/libreoffice/`, because the engine starts as a child process; the shell passes that directory to the sidecar, which resolves those packages from disk, and the embedded runtime excludes them. The package contains no development TypeScript, source maps, tests, or documentation. A third-party plugin stays on disk below the user's Harness profile and may load its private dependencies while sharing the packaged Cordis and Harness Service Definition instances.
...
The shell displays its embedded startup page immediately, accepts only the sidecar's exact loopback origin, denies popups and unexpected top-level navigation, and exposes no Tauri shell, filesystem, or general invoke API to the Web UI. Before authenticated loopback navigation, it removes earlier Harness authentication cookies for that host so repeated random-port launches cannot overflow the backend's request-header limit; unrelated WebView cookies remain untouched. A second launch focuses the existing window. On Windows, the window close button saves its state and hides it to the system tray; click the tray icon or choose **Show DeepSeek Harness** to restore it, and choose **Exit** from the tray menu to stop the application. Exit sends a protocol-level shutdown request and waits for Harness disposal; the Windows Job Object and macOS process group are timeout fallbacks. macOS keeps its normal window-close behavior.
