...
| `resolverInterceptionRanges` | `[]` | 部署的解析器拦截把域名映射到的 IPv4 CIDR（例如 fake-IP 池，由 TUN 把连接转发给代理）；条目必须落在 `198.18.0.0/15` 或 `240.0.0.0/4` 内 |
...
提供方保持请求匿名且有界：只接受不含内嵌凭据且不超过 2,048 个字符的 `http:` 与 `https:` URL。它只解析一次主机名；只要结果中有任何 IPv4 或 IPv6 地址既不是公共单播地址、也不在已声明的 `resolverInterceptionRanges` 池内，就拒绝整个结果，并把连接固定到已校验的地址集合。声明某个范围等于部署自己确认解析器会在该范围内拦截域名——fake-IP VPN 把真实目的地放在隧道之后——因此可接受的池在公网上不可路由、也永远不会指向本机服务；私有、回环、链路本地与运营商级 NAT 地址仍然一律拒绝。IP 字面量永远不会被当作拦截结果，因为没有任何解析过程把它放进去。IPv6 检查会发现活动 DNS64 前缀，并拒绝指向非公开 IPv4 的转换地址。每次同源重定向都会重复解析与固定；跨源重定向会失败并要求重新调用。提供方还强制执行字节、字符、跳数和时间上限，拒绝不支持的内容类型，并发送显式产品 `User-Agent`。
