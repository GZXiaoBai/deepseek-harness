...
describe('first-party Session format catalog', () => {
  it.each(['default', 'selection', 'inferred'])('preserves historical permission origin %s through the current format', (origin) => {
    ...
    const restore = createSessionFormatCatalogWithChildren([])
      .createRestore(header, { recovery: 'strict', validation: 'current' })
    for (const row of rows) restore.decodeRow(row)
    const migrated = restore.finish()
    expect(migrated.header.version).toBe(SESSION_FORMAT_VERSION)
