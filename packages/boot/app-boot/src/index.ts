 ...
 * @param bareModuleBaseUrl - optional installed-host base for the named embedded packages.
 * @param bareModulePackages - package roots routed to the host; other names resolve beside the configuration file.
...
export async function mountRootInclude(
  ...
  const hostedPackages = new Set(bareModulePackages)
  ctx.loader.builtins.include = bareModuleBaseUrl === undefined
    ...
    : class HostResolvedRootInclude extends Include {
      override import(name: string, getOuterStack?: () => string[]): unknown {
        ...
        if (packageName === undefined || !hostedPackages.has(packageName)) return super.import(specifier, getOuterStack)
 ...
 * bare package names resolve there by default. Packages listed in
 * `bareModulePackages` resolve against `bareModuleBaseUrl` for embedded runtimes. The bootstrap include
 ...
 * @param bareModuleBaseUrl - optional installed-host base for the named embedded packages.
 * @param bareModulePackages - package roots routed to the host; other names retain configuration-local resolution.
...
export async function boot(
  ...
  try {
    ...
    await mountRootInclude(ctx, absoluteConfigPath, patches, bareModuleBaseUrl, bareModulePackages, binName)
