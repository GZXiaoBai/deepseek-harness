...
- **Embedded package routing.** Embedded hosts supply `bareModuleBaseUrl` together with the package roots in `bareModulePackages`. Only those packages and their subpaths resolve from the host; other package names retain configuration-local lookup, including plugins installed in the active profile.
