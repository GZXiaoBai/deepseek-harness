...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). Browsers use this server directly; the [first macOS Desktop wrapper](../../../.agents/notes/implemented/feature/2026-08-16-macos-desktop-app.md) also starts it on `http://127.0.0.1:<ephemeral-port>` and loads that same origin in Electron. A future `file://` plus IPC host is a distinct transport with its own asset, request, lifecycle, and security responsibilities. This package never prints; the URL line belongs to the shell.
