...
该包不了解任何 harness 概念，也不提供任何文件服务：`/api` HTTP 桥接与下行 WebSocket 是 connection 插件的 route，插件 bundle 与 HMR（热模块替换）事件流是 modules／hmr 插件的 route，dist 服务则属于 fallback 持有者。upgrade handler 拥有协议握手与连接内容；webserver 只交付原始 socket 与 request。`host` 只接受 `127.0.0.1`（默认安全姿态）和 `0.0.0.0`（有意向网络开放）。浏览器会直接使用该服务器；[Tauri Desktop 外壳](../../../.agents/notes/implemented/architecture/2026-08-21-tauri-desktop-sidecar.zh.md)会在 `http://127.0.0.1:<ephemeral-port>` 上启动打包的 sidecar，并在 WebView 中加载该精确来源。未来采用 `file://` 加 IPC 的 host 是一种独立传输方式，并拥有自己的资源、请求、生命周期和安全职责。该包从不打印内容；URL 行属于 shell。
