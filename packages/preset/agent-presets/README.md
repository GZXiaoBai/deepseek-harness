...
| `resolvedPackages` | `[]` | Bare package roots an embedding host resolver supplies without on-disk `node_modules` entries |
...
Ordinary deployments omit `harnessBase` and `resolvedPackages`. An embedder sets `harnessBase` when it has an installed package tree, and lists the package roots supplied only by its module resolver in `resolvedPackages`. Health checks every other bare package through the runtime's plugin package service from the roster's Loader base when available, otherwise against the harness's on-disk `node_modules`. Profile-installed plugins remain discoverable even when the embedded harness uses another base; absent third-party plugins remain broken.
