...
Ordinary deployments omit `harnessBase` and `resolvedPackages`. An embedder sets `harnessBase` when it has an installed package tree, and lists the package roots supplied only by its module resolver in `resolvedPackages`. Health still checks every other bare package against the harness's on-disk `node_modules`, so a user preset naming an absent third-party plugin remains broken.
