...
describe('the preset roster', () => {
  ...
  it.each(['embedded-package', '@embedded/package'])('accepts embedded %s subpaths while checking other packages', async (name) => {
    const root = await mkdtemp(join(tmpdir(), 'dsh-preset-embedded-package-'))
    roots.push(root)
    await mkdir(join(root, 'embedded'))
    await writeFile(join(root, 'embedded', COMPOSITION_FILE), `- id: package\n  name: '${name}/plugin.js'\n`)
    await mkdir(join(root, 'missing'))
    await writeFile(join(root, 'missing', COMPOSITION_FILE), '- id: package\n  name: missing-package/plugin.js\n')
    const scoped = await harness({
      default: 'embedded', roots: [{ path: root, trust: 'system' }],
      includeShippedRoot: false, includeUserRoot: false, resolvedPackages: [name],
    })
    const packageOf = vi.spyOn(scoped.pluginPackages, 'packageOf').mockReturnValue(undefined)

    const listed = await scoped.agentPresets.list()
    expect(listed.find(preset => preset.id === 'embedded')?.broken).toBeUndefined()
    expect(listed.find(preset => preset.id === 'missing')?.broken)
      .toBe('row "package" names a plugin that cannot be resolved: missing-package/plugin.js')
    expect(packageOf).toHaveBeenCalledExactlyOnceWith('missing-package/plugin.js', pathToFileURL(FIXTURES).href + '/')
  })
