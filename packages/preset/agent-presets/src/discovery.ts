...
export async function scanRoot(
  ...
  const packageResolves: PackageResolves = (specifier, base) => {
    const root = specifier.split('/').slice(0, specifier.startsWith('@') ? 2 : 1).join('/')
    return resolvedPackages.has(root) || resolves(specifier, base)
  }
  ...
  for (const child of children) {
    ...
    const broken = await isFile(path)
      ? await compositionProblem(path, harnessBase, packageResolves)
...
export async function discoverPresets(
  ...
  for (const root of roots) {
    for (const preset of await scanRoot(root, harnessBase, resolves, resolvedPackages)) {
