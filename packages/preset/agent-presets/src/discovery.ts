 ...
 * @param resolvedPackages - package roots supplied by an embedding host resolver.
...
async function rowResolves(
  ...
  if (row.kind === 'package') {
    const root = row.specifier.split('/').slice(0, row.specifier.startsWith('@') ? 2 : 1).join('/')
    return isBuiltin(row.specifier) || resolvedPackages.has(root) || packageInstalled(row.specifier, harnessBase)
  }
  ...
  for (const [index, entry] of rows.entries()) {
    ...
    if (row.group === true) {
      found.push(...await unresolvableRows(
        row.config as readonly unknown[], presetBase, harnessBase, resolvedPackages, positional,
      ))
      continue
    }
    if (await rowResolves(classifyRowSpecifier(row.name), presetBase, harnessBase, resolvedPackages)) continue
...
async function compositionProblem(
  ...
  const unresolvable = await unresolvableRows(
    rows as readonly unknown[], presetBase, harnessBase, resolvedPackages,
  )
...
export async function scanRoot(
  ...
  for (const child of children) {
    ...
    const broken = await isFile(path)
      ? await compositionProblem(path, harnessBase, resolvedPackages)
...
export async function discoverPresets(
  ...
  for (const root of roots) {
    for (const preset of await scanRoot(root, harnessBase, resolvedPackages)) {
