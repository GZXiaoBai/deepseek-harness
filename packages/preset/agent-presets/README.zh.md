...
| `resolvedPackages` | `[]` | 嵌入式宿主解析器提供、但磁盘 `node_modules` 中不存在的裸 package 根名称 |
...
普通部署省略 `harnessBase` 与 `resolvedPackages`。嵌入器存在已安装 package 树时设置 `harnessBase`，并在 `resolvedPackages` 中列出仅由其模块解析器提供的 package 根名称。健康检查会从 roster 的 Loader base 通过运行时的插件包服务检查其他裸 package；该服务不存在时，则检查 harness 的磁盘 `node_modules`。即使嵌入式 harness 使用其他 base，安装在 profile 中的插件仍然可以被发现；缺失的第三方插件仍会被标为损坏。
