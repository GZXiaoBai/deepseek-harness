...
declare module '@deepseek-ai/cordis' {
  interface Context {
    ...
    /** Optional host lookup for package metadata embedded outside the profile filesystem. */
    clientPackageJsonResolver: ClientPackageJsonResolver
  }
}

/** Host adapter for package metadata stored in an embedded runtime. */
export interface ClientPackageJsonResolver {
  /**
   * Resolve one package's exported manifest.
   * @param spec - Bare package name from a Loader entry.
   * @returns An absolute readable path, or undefined when the host does not own the package.
   */
...
/**
 * Builds the package metadata resolver used by the client-module scan.
 * Host-owned packages resolve through the optional adapter; all other names
 * continue from the config tree so user-installed plugins keep private peers.
 * @param profileBaseUrl - Config-tree URL used for user-installed plugins.
 * @param hostResolver - Optional adapter for packages embedded by the host.
 * @returns A resolver for each package's exported package.json.
 */
export function createClientPackageJsonResolver(
  profileBaseUrl: string,
  hostResolver?: ClientPackageJsonResolver,
): (spec: string) => string {
  const profileRequire = createRequire(profileBaseUrl)
  return (spec) => {
    const hostPath = hostResolver?.resolve(spec)
    return hostPath ?? profileRequire.resolve(`${spec}/package.json`)
  }
}
...
export class ClientModuleRegistry extends Service {
  ...
  constructor(ctx: Context) {
    ...
    this.resolvePkgJson = createClientPackageJsonResolver(
      ctx.baseUrl,
      ctx.get('clientPackageJsonResolver'),
    )
