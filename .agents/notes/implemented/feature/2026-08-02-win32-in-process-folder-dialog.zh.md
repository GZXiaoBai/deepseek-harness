...
- 真实对话框的渲染与选择路径仍需在 Windows 上手动检查。Windows 产物验收会分别通过复制后的未打包可执行文件和已安装可执行文件启动随包 worker，自动关闭真实对话框，并要求收到终态取消结果后才接受对应产物。
