...
The application enforces one Electron instance and one backend. A second launch focuses the existing window without starting another backend. Startup failure stays in a script-free local document and offers retry, log-directory access, or quit.
