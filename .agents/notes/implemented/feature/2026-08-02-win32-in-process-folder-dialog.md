...
- Real dialog rendering and selecting a path stay a manual Windows check. Windows package acceptance launches the packaged worker through both the copied unpacked executable and the installed executable, auto-closes the real dialog, and requires its terminal cancellation before accepting either artifact.
