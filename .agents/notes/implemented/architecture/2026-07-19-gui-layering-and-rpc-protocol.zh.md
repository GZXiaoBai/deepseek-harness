...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - [首个 macOS Electron 应用](../feature/2026-08-16-macos-desktop-app.md)通过私有回环 HTTP 来源包装 `dsh web`。未来的 IPC fetch 载体仍是一套独立传输架构。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器 HTTP 与首个 macOS Desktop 包装层；零 workspace 依赖（注册表经结构注入到达）；未来的 IPC host 会替代而不是别名化它 |
