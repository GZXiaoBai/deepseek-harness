...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - The [first macOS Electron application](../feature/2026-08-16-macos-desktop-app.md) wraps `dsh web` over a private loopback HTTP origin. A future IPC fetch carrier remains a separate transport architecture.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser HTTP plus the first macOS Desktop wrapper; zero workspace dependencies (the registry arrives by structural injection); a future IPC host replaces rather than aliases it |
