...
The [Tauri packaging configuration](../../../../apps/desktop/src-tauri/tauri.conf.json), [native desktop entry](../../../../apps/desktop/src-tauri/src/main.rs), and [sidecar entry](../../../../apps/desktop/src/sidecar-bin.ts) are the current inspection inputs. The Electron-specific mechanisms proposed below require reassessment against this Tauri implementation before implementation. Source inspection identifies cleanup candidates; installed-artifact observation must establish the complete supported inventory before implementation is accepted. No uninstall experiment has yet established an exhaustive Windows or macOS inventory.
...
OS-owned security/audit history and user-created files cannot be covered by a promise of erasing all traces. The [Tauri desktop decision](../../implemented/architecture/2026-08-21-tauri-desktop-sidecar.md) owns current packaging and data ownership; this proposal does not supersede it.
